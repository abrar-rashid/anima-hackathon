...
export function retrospectiveEvent(resource: Pick<SimResource, 'data' | 'patientId'>): {
  time: number; stage: string; role: string; completed: boolean
} | null {
  ...
  try {
    ...
    return { time, stage: meta.stage, role: typeof meta.responsible_role === 'string' ? meta.responsible_role : 'Not supplied',
