...
import { PixelTownCanvas, type TownSimulationData } from '@/components/town/PixelTownCanvas'
import {
  CommandCockpit,
  type CockpitData,
  type LoopActReceipt,
...
type LiveSimulation = CockpitData & {
  world?: string
  live?: boolean
  recentEvents?: TownSimulationData['recentEvents']
  districts?: TownSimulationData['districts']
}
...
export default function Home() {
  ...
  const townData: TownSimulationData | null = simData
    ? {
        clock: simData.clock,
        patient: {
          id: simData.patient.id,
          name: simData.patient.name,
          recentLab: simData.patient.recentLab,
      ...
      <header className={styles.header}>
        ...
        <div className={styles.aside}>
          ...
          <div className={styles.livePill}>
            ...
            <span>Live Sim: {simData?.world ?? 'team-ea32f6302052'}</span>
          </div>
        </div>
      </header>
      ...
      <main className={viewMode === 'town' ? `${styles.main} ${styles.mainTown}` : styles.main}>
        {loading && !simData ? (
          ...
          <>
            ...
            {viewMode === 'town' && townData ? (
              <PixelTownCanvas
                data={townData}
                onAdvanceClock={handleAdvanceClock}
                onCloseLoop={() => void handleExecuteAct('auto_close')}
              />
            ) : null}
